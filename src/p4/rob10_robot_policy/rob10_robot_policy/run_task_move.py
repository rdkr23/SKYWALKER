...
class ReachPolicy(Node):
    ...
    def get_current_base_position(self):
        ...
        try:
            self.rotated_frames()
            transform: TransformStamped = self.tf_buffer.lookup_transform(
                "link_eef", "link_base", rclpy.time.Time()
            )

            translation = transform.transform.translation

            transform: TransformStamped = self.tf_buffer.lookup_transform(
                "eef_rotated", "base_rotated", rclpy.time.Time()
            )
            rotation = transform.transform.rotation

            current_ee_pose = np.array([translation.z, -translation.y, translation.x])
            current_ee_quat = ros_to_sim_quat_wxyz(np.array([rotation.w, rotation.x, rotation.y, rotation.z])) * -1
            self.robot.orig_pose = np.concatenate([current_ee_pose, current_ee_quat])


            # Pass the current end-effector position to the robot policy
            final_fucking_pose = np.concatenate((current_ee_pose, current_ee_quat))
            return final_fucking_pose
