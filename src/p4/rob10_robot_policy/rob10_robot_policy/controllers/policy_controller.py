...
from rob10_robot_policy.utils.config_loader import parse_env_config, get_physics_properties, get_robot_joint_properties
...
class PolicyController:
    ...
    def load_policy(self, policy_file_path, policy_env_path) -> None:
        ...
        print("\n=== Policy Loaded ===\n")
        # If the policy contains running statistics (normalization), print their shape for debugging
        try:
            if hasattr(self.policy, 'running_mean'):
                rm = self.policy.running_mean
                rv = getattr(self.policy, 'running_var', None)
                print(f"Policy contains running_mean of shape: {tuple(rm.shape)}")
                if rv is not None:
                    print(f"Policy contains running_var of shape: {tuple(rv.shape)}")
        except Exception:
            # Some TorchScript modules expose attributes differently; ignore failures here
            pass
    ...
    def _compute_action(self, obs: np.ndarray) -> np.ndarray:
        ...
        """
        # Sanity check: if the policy has running_mean (normalizer), ensure obs size matches
        try:
            if hasattr(self, 'policy') and hasattr(self.policy, 'running_mean'):
                try:
                    expected = int(self.policy.running_mean.shape[0])
                except Exception:
                    expected = None
                if expected is not None and obs.size != expected:
                    raise RuntimeError(
                        f"Observation size mismatch: policy expects {expected} but obs has {obs.size}. "
                        "Check your env.yaml 'num_observations' and observation ordering/format."
                    )
        except RuntimeError:
            raise
        except Exception:
            # Non-fatal if attribute access fails
            pass
