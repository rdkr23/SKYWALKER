    ...
    tf_broadcaster(this)
{
    ...
    camera_id = declare_parameter("camera_id", "", descr("Unique ID for the node", true));
                            ...
                            const sensor_msgs::msg::CameraInfo::ConstSharedPtr& msg_ci)
{
    ...
    for(int i = 0; i < zarray_size(detections); i++) {
        ...
        if(estimate_pose != nullptr && calibrated) {
            ...
            tf.child_frame_id = tag_frames.count(det->id) ? tag_frames.at(det->id) : camera_id + std::string(det->family->name) + ":" + std::to_string(det->id);
